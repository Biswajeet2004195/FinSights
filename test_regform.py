...
def login():
    ...
    def go():
        u = users()
        if e.get() not in u or not verify_password(u[e.get()]["password"], p.get()):
            return messagebox.showerror("Login Failed", "Invalid email or password.")
        
        user_data = u[e.get()]
        pwd = user_data["password"]
        migrated = False
        if not pwd.startswith('$2') and not pwd.startswith('pbkdf2:$'):
            user_data["password"] = hash_password(p.get())
            migrated = True
        if migrated:
            save(u)
            
        fade_out_window(root, lambda: launch_dashboard_with_fade(root, user_data["name"], e.get()))
...
def signup():
    ...
    def reg():
        u = users()
        if not all([n.get(), e.get(), p.get(), c.get()]):
        ...
        u[e.get()] = {
            "name": n.get(),
            "password": hash_password(p.get())
        }
        save(u)
        messagebox.showinfo("🎉 Welcome!", f"Hello {n.get()}, your account is ready!")
...
def forgot():
    ...
    def rst():
        ...
        if p.get() != c.get():
            return messagebox.showerror("Mismatch", "Passwords do not match.")
        u[e.get()]["password"] = hash_password(p.get())
        save(u)
